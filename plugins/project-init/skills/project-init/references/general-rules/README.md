...
| `worktree-isolation.md` | Assume other Claude sessions share the repo. Look before you edit (`git worktree list`, `git status`, `git log`); work in your own git worktree on your own branch, never in the shared checkout; land work by PR and merge only on owner approval, after a collision check. Also the shared-checkout hygiene that stops sessions overwriting each other: never `git add -A`, shared index files are append-only, and claim a sequential identifier by pushing it first because parallel agents WILL collide on numbers. Any relaxation must use an objective boundary, not a self-judged "small edit". |
