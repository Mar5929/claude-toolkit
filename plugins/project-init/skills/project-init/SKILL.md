...
On Windows or macOS, offer optional `voice-reply` for spoken replies when useful
to the owner. Route to `machine-sync` or the `voice-reply` plugin's
`voice-setup` skill. It is installed for the computer's user across projects,
with each new chat OFF; do not configure it in this project.
