...
| File | What it does |
|---|---|
| `where-persistent-information-belongs.md` | Put active work wherever its work item is tracked, the project's current state and handoff in `knowledge/current.md`, raw client material with delivery artifacts, outside research in the mapped reference area, agent instructions in rules, reusable processes in skills, approved behavior in specifications, and durable memory in one of the four record types: facts, decisions, events, and patterns. Uses short approval bullets and prevents copies. |
...
| `parallel-agent-sessions.md` | Sharing a repo with other live sessions, in one rule: where you work and how you behave. Look before you edit (`git worktree list`, `git status`, `git log`); work in your own worktree on your own branch and never in the shared primary checkout; never `git add -A`; keep shared-file edits narrow and preserve unrelated entries while allowing owner-approved project-knowledge maintenance; rebuild a derived artifact with `memory.mjs rebuild-views` instead of merging it; claim a sequential identifier by pushing it first, because parallel agents WILL collide on numbers; never touch another session's branch or commit its uncommitted work; land by PR, merge only on owner approval and only after a collision check. Written after one session caused a work-item number collision, had its staged files swept into another session's commit twice, and reordered a shared backlog. Absorbed the separate `worktree-isolation` rule. |
...
| `steer-to-the-goal.md` | Find the real goal behind the request, name it early, hold it all session, own the steering, and end every turn with the next step. Push back directly when a request is risky, over-built, or aimed at the wrong target, then do it the owner's way once decided. Persistent direction goes to the work tracker, and to an approved decision or fact record when it has to outlive the tracker; live next actions stay wherever the work item is tracked and in `knowledge/current.md`. |
...
The current procedure comes from the `second-brain` plugin, version 2, as
packaged skills, tools, and fail-open hooks. Its shape is four record types,
`knowledge/current.md`, the project map, and approved specifications. Do not
restore or duplicate the retired large rule, the verifier, the generated index,
the seven version 1 memory folders, or per-folder indexes in this library.
