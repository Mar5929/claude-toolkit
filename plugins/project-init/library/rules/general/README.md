...
| `wrap-up-ritual.md` | At approved completion points and natural stopping points after meaningful work, align specifications, code, and tests, propose durable memory updates, require the librarian for approved writes, review changed memory against the latest project state before merge, update the work-tracker handoff when relevant, and use the project's Git workflow. Ordinary responses, trivial actions, and unfinished handoffs with no settled durable result do not trigger a memory review. |
