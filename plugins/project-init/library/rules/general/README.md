...
| File | What it does |
| --- | --- |
| `knowledge-direct-commit.md` | Path-scoped to `knowledge/**`, `prds/**`, `PROJECT.md`, `docs/**`, and `**/README.md`. Authorized documentation-only changes use an isolated save worktree and publish straight to the default branch; behavior files use a worktree and pull request. Stage by name, never force-push or alter another session's checkout, and verify the remote. The steps are in the `publish-docs` skill (git-workflows). Applies without knowledge enabled. |
