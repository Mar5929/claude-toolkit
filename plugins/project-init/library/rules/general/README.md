...
| File | What it does |
|---|---|
| `capture-the-thinking.md` | Never leave the thinking only in chat. The goal, the why, requirements, edge cases, decisions, open questions, and constraints get written into their canonical home while work happens. Carries the routing table for the work tracker and `knowledge/`, the write-it-before-you-build step, and the vanished-chat test. The installed `remember` skill owns durable approval and the completion review. |
...
| `steer-to-the-goal.md` | Find the real goal behind the request, name it early, hold it all session, own the steering, and end every turn with the next step. Push back directly when a request is risky, over-built, or aimed at the wrong target, then do it the owner's way once decided. Durable direction goes to `knowledge/memory/planning/`; live next actions remain in work-tracker. |
