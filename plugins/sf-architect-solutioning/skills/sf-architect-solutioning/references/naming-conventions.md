...
Standard naming conventions for all Salesforce engagements. Client conventions in the project's AGENTS.md override these where they differ.
...
|---|---|---|
| Feature | `feature/{ticket-id}-{short-description}` | `feature/PROJ-42-account-validation` |
| Bugfix | `bugfix/{ticket-id}-{short-description}` | `bugfix/PROJ-67-duplicate-trigger-fix` |
| Hotfix | `hotfix/{short-description}` | `hotfix/critical-integration-fix` |
| Release | `release/v{version}` | `release/v1.2` |

Use the id scheme of the project's own ticketing system. Project rules about branching (e.g. worktree-only branches) override these conventions.
