...
| `plain-english-artifacts.md` | Every artifact an agent makes for a person to read is written in plain, explicit technical English: no jargon, no preamble, no commentary, not a story, none of the patterns that read as machine-written. Covers the words inside documents, diagrams, charts, slides, prompts, and issue text, whoever writes them. |
...
inside the things Claude makes, not how Claude talks here, and it is a rule
because the style never reaches a helper agent, a Codex session, or a file.
