...
| `plain-english-artifacts.md` | Every diagram, chart, dashboard, visualization, slide deck, or generated document an agent makes uses plain, explicit technical English in every title, label, legend, caption, and note: no jargon, no preamble, no commentary, not a story, none of the patterns that read as machine-written. Chat, code, README files, and issue text are not covered. |
