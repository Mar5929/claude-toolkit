...
- 2026-09-26: After the third review: the shell reader treats an unquoted `#`
  at the start of a word as a comment to the end of the line; auto-allow needs
  a one-line command of plain characters and resolves a bare `flow` through
  `CLAUDE_PLUGIN_ROOT`, because a hook's PATH does not include the plugin's
