...
| handoff | session-skills | Update the active tracker when present, invoke the project's remember workflow when available, then draft a fresh-session prompt that carries a fixed AI-generated notice on its first line, then the goal, then anything unsaved, and is checked against the repository | `/handoff`, `/handoff check`, "write a handoff", "I'm going to clear context" |
