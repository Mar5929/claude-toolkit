...
Voice conversations use the external
[VoiceMode plugin](https://github.com/mbailey/voicemode), installed separately
for Claude Code. It is not a toolkit plugin.
...
| knowledge-review | second-brain | Review duplicates, contradictions, obsolete content and selection feedback | "review project knowledge" |
...
| publish-docs | git-workflows | Prepare an authorized documentation save in an isolated worktree, then check, stage by name, commit, push to the default branch, and verify | `/publish-docs`, "save this document" |
...
| handoff | session-skills | Update the active tracker when present, invoke the installed knowledge-save or legacy remember workflow, preserve requested continuation in newest-first Session handoffs when current working memory is configured, then show a checked fresh-session prompt that carries a fixed AI-generated notice on its first line, then the goal, then anything unsaved, and is checked against the repository | `/handoff`, `/handoff check`, "write a handoff", "I'm going to clear context" |
...
- **Project knowledge runtime sources**: the four procedures and explicit-only compatibility routes under
...
- **protocol-guard versus the second-brain command hooks.** The command hooks
  ...
  `memory-reminder.mjs` its turn-review line (CW, K4 and K6). The PreToolUse
  hooks read the `TOOLKIT_PROTOCOL_ENGINE` variable instead: while K7 runs,
  `work-item-close.mjs` and `save-reminder.mjs` skip their hold-once, and the
  knowledge-only branch message stays. With the variable off, or after engine
  errors, the command hooks run in full. Keep both.
...
- **session-summary versus work-tracker and persistent review.** All three
  ...
  there rather than into a summary. The `knowledge-save` review is the moment that
