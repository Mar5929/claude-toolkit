...
Optional [voice-reply](../plugins/voice-reply/README.md) provides Windows and
macOS speech output for both hosts. Its `voice-setup` skill installs at user
scope through the same route offered by `machine-sync`; `toolkit-voice` is the
installed help skill. No project setup is required.
