...
folder detail reaches an agent when it opens that folder instead of loading in
every session.
...
- **Four kinds of folder are skipped**, and every skip is recorded in the setup
  ...
  with an obvious name and no conventions to state.
...
- **Four things stay in the root file:** how to talk to the owner, the pointers
  to the most dangerous rules, the memory routing section when the memory system
  is installed, and the codemap lines themselves.
