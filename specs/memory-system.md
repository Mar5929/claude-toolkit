...
The design for saving and reading persistent information in a project: the two
folders, what goes in each, who may write to them, what every file looks like,
and how files link to each other.
...
What a thing must do. One file per capability, named after the capability.

### What goes in `memory/`, and which folder

| Folder | Put a file here when | Do not put it here when |
|---|---|---|
| `memory/context/` | Something about the situation shapes several pieces of work: who is involved, what the project is up against, a limit that comes from outside. | It is what someone is working on right now, or what is blocked. That belongs in the work tracker. |
...
Files sit directly inside these seven folders. There is no third level of
folders below them.
...
### The one rule that is always loaded

An agent may never write into `specs/` or `memory/` without first showing the
user the exact words and getting a yes from the user.

That sentence is the only part of this document loaded into every session.
...
An agent does not load every file. It reads the folder names above to work out
which folder holds the kind of thing it needs, lists the file names in that
folder and in `specs/`, searches the text inside them for the subject, opens
only what the task needs, and follows a link only when the linked file matters
to that task.
...
1. **Search first.** Read the file names in `specs/` and `memory/`, and search
   the text inside them for the fact about to be saved.
...
5. **Show the user, then stop.** The user replies with numbers: keep, cut, or
   edit.
6. **Write only what the user kept**, then state exactly which files were
   written and what was cut.
...
| Scripts that build index files or check file shape | They keep files tidy so an agent can find things. Searching already finds things. |
| The memory instructions copied into `CLAUDE.md` and `AGENTS.md` | Replaced by the one rule that is always loaded. |
