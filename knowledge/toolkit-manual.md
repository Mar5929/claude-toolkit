...
| Required workflow checks | Claude Code only. Refuse a tool call, or hold a final reply once, when a required step did not happen. | [protocol-guard](../plugins/protocol-guard/README.md); turned on in `.claude/settings.json` |
| Commit-time knowledge check | Every host. A Git pre-commit hook refuses a commit whose staged knowledge files fail the knowledge checker. | [knowledge manual](knowledge-manual.md); installed in each clone by knowledge setup and `project-sync` |
