...
- In Claude Code with the `protocol-guard` required workflow checks on, each of these three actions is refused until `knowledge-save` was opened in the same turn, instead of held once (check K7). Closing a work item is also refused until the `work` skill was opened in the same turn (P2), and a merge until `merge-and-clean-up` was opened in the session (P3). The checks prove the skill was opened, not that the review was good or that the owner approved the close. The hold-once above stays as the backup when the checks do not run, and in Codex. Approved by Mike Rihm on 2026-09-22 in work item [#396](https://github.com/Mar5929/claude-toolkit/issues/396), decision 10; built in #396.
