...
import { beginTurn, top, stepOf, missingForStep, describe } from './runner.mjs';
...
const SAVE_PHRASE = /^\s*(save this|remember this|remember that)\b/i;
const READ_ONLY_TOOLS = new Set(['Read', 'Glob', 'Grep', 'LS', 'NotebookRead', 'ToolSearch', 'WebFetch', 'WebSearch', 'TaskList', 'TaskGet', 'TaskOutput']);
...
function protectedMessage(area) {
  return `Files under ${area}/ change only through the flow command. Use \`flow ${area === 'memory' ? 'route remember' : 'item ...'}\` instead. Run \`flow help\` for the commands.`;
}
...
function shellWriteReason(root, cwd, segments) {
  for (const seg of segments) {
    for (const target of seg.redirects) {
      ...
      if (area) return protectedMessage(area);
    }
...
export function preToolUse(input, env = process.env) {
  ...
  if (WRITE_TOOLS.has(tool)) {
    ...
    if (area) return deny(protectedMessage(area));
  }
