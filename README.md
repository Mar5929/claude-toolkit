  ...
  plugins/
    ...
    hooks-library/                ← plugin: hooks that check a moment mechanically
      ...
      hooks/
        ...
        style-handshake.mjs       ← requests a silent style re-read on each user message
    ...
    protocol-guard/               ← plugin: required workflow checks (Claude Code only)
      README.md
      .claude-plugin/plugin.json  ← no .codex-plugin: Codex cannot run function hooks
      hooks/
...
| **[git-workflows](plugins/git-workflows/README.md)** | Four parallel-session-safe git lifecycle skills: `pull-latest` gets current without rewriting history, `reset-to-remote` mirrors the remote behind confirmation, `merge-and-clean-up` lands an approved PR before removing only its completed workspace, and `publish-docs` uses an isolated worktree to land an authorized documentation-only save on the default branch. | Install and go |
