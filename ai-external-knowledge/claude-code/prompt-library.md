...
export const labels = {
  ...
  needsLabel: "Needs",
  needs: {
    tracker: "your issue tracker added as a [claude.ai connector](https://code.claude.com/docs/en/mcp#use-mcp-servers-from-claude-ai) or [MCP server](https://code.claude.com/docs/en/mcp).",
    gh: "the [gh CLI](https://cli.github.com) authenticated, or GitHub added as a [claude.ai connector](https://code.claude.com/docs/en/mcp#use-mcp-servers-from-claude-ai).",
    browser: "a way for Claude to render and screenshot the result. The [Desktop app](https://code.claude.com/docs/en/desktop#preview-your-app) has this built in. In the terminal, install the [Chrome extension](https://code.claude.com/docs/en/chrome) or a Playwright [MCP](https://code.claude.com/docs/en/mcp) server.",
    db: "your data warehouse or log store added as a [claude.ai connector](https://code.claude.com/docs/en/mcp#use-mcp-servers-from-claude-ai) or [MCP server](https://code.claude.com/docs/en/mcp)."
  }
};
...
export const text = {
  ...
  "plan-a-multi-file": {
    ...
    teaches: "Adding \"don't edit yet\" separates exploration from changes, so you see the approach before any code moves. To make plan-first the default on every prompt, press Shift+Tab for [plan mode](https://code.claude.com/docs/en/permission-modes#analyze-before-you-edit-with-plan-mode)."
  },
  "draft-a-spec-by": {
  ...
  "turn-a-meeting-into": {
    ...
    teaches: "Skip the transcription step. Claude pulls action items from the unstructured input and writes them straight into your tracker via [MCP](https://code.claude.com/docs/en/mcp), so you review the tickets, not the transcript.",
  ...
  "migrate-a-pattern-across": {
    ...
    teaches: "Describe the old pattern and the new one. Asking Claude to identify every place first means the call sites are listed in the response, so you can check none were missed. For a migration across many files, run [/batch](https://code.claude.com/docs/en/commands). Claude splits the work into units for you to approve, then background subagents make the changes and open one pull request per unit."
  },
  ...
  "run-a-security-review": {
    ...
    teaches: "A [subagent](https://code.claude.com/docs/en/sub-agents) runs the audit in its own context window and reports back a summary, so a long security review doesn't fill up your main session. The built-in general-purpose subagent handles this without extra setup.",
  ...
  "turn-a-correction-into": {
    ...
    teaches: "A correction in chat isn't shared with your team. A rule in the project's [CLAUDE.md](https://code.claude.com/docs/en/memory) is shared once you commit it, and Claude reads it at the start of every session.",
  ...
  "turn-a-recurring-task": {
    ...
    teaches: "Name the steps once; reuse them as a command. Claude writes a [skill](https://code.claude.com/docs/en/skills) anyone on your team can run."
  },
  "add-a-hook-for": {
    title: "Add a hook for repeat behavior",
    teaches: "Hooks make a behavior automatic instead of something you have to remember to ask for. Describe the trigger and action and Claude writes the [hook](https://code.claude.com/docs/en/hooks) configuration."
  },
  "connect-a-tool-with": {
    title: "Connect a tool with MCP",
    teaches: "Connect the source once instead of pasting data every session. After [MCP](https://code.claude.com/docs/en/mcp) setup, Claude reads from the tool directly when you ask about it."
  },
  "capture-what-to-remember": {
    title: "Capture what to remember for next time",
    teaches: "Ask before you forget. Claude knows what it had to figure out this session and proposes [CLAUDE.md](https://code.claude.com/docs/en/memory) entries so the next session starts with that context."
  }
};
